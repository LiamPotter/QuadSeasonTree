...
public class AutoSceneOpener:EditorWindow
{
    ...
    public static void ShowWindow()
    {
        ...
        GetWindow(typeof(AutoSceneOpener)).minSize = new Vector2(300, 300);
    ...
    { 
        ...
        //GUI.contentColor= variables.sOpenPopup.windowTint;
        showSceneList=true;
        ...
        GUIStyle style1 = new GUIStyle(); //Style for Options Symbol
        ...
        //style0.normal.background = (Texture2D)AssetDatabase.LoadAssetAtPath("Assets/Gizmos/TriTools/greyBackground.png", typeof(Texture2D));
        //style0.normal.background.wrapMode = TextureWrapMode.Clamp;
        #region Options Button Alignment      
        float minWid;
        float maxWid;
        style1.CalcMinMaxWidth(content2, out minWid, out maxWid);
        style1.fixedWidth = minWid;
        style1.alignment = TextAnchor.UpperRight;
        style1.padding.right = 1;
        style1.padding.top = 4;
        style1.normal.background = null;
        style1.hover.textColor = Color.red;
        #endregion
        ...
        else style0.normal.textColor = Color.black;

        EditorGUILayout.BeginHorizontal();
        GUILayout.Label("Scenes Amount: "+ variables.autoSceneAmount.ToString(), style0);
        ...
        if (GUILayout.Button(variables.sOpenPopup.windowTheme ==SceneOpenerPopup.Theme.Default? content2:content3, style1))
        {
            Rect buttonRect =new Rect(Screen.width+0.1f,-35f,minWid,style1.CalcHeight(content2,minWid));
        ...
        if (EditorApplication.timeSinceStartup > 1f)
        {
            ...
            ListIterator("Auto_Open_Scenes", ref showSceneList, serializedVarObj, style0);
            if (difference == 0)
            {
    ...
    public void ListIterator(string propertyPath,ref bool visible,SerializedObject serializedObject,GUIStyle style)
    {
        SerializedProperty listProperty = serializedObject.FindProperty(propertyPath);
        visible = EditorGUILayout.Foldout(visible, listProperty.name,style);
        if (visible)
        {
            EditorGUI.indentLevel++;
